...
class PlotConfig():
    ...
    def __init__(self, optimization, dataframe, bins):
    ...
    def _parameters_filter(self, data) -> Union[List[pd.DataFrame], List[pd.Series]]:
        """
    ...
    def plot_errorbars(self,
        ...
        for data, config in zip(data_metric, plot_configs):
            ...
            # Extração de configurações com valores padrão (fallback)
            label = config.get('label', 'Series')
            fmt = config.get('fmt', 'o')
            color = config.get('color', None)
            
            # Preparação dos argumentos de plotagem
